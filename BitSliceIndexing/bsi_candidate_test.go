...
func TestBatchEqualDynamicAllocationManyBitplanes(t *testing.T) {
	// Create a BSI with 130 bitplanes (more than 128!) to force dynamic allocation branch
	bsi := NewDefaultBSI()
	...
	// Column 1: value is 1<<129 (so only plane 129 has it)
	bsi.bA[129].Add(1)
	// Column 2: value is 1<<3 (so only plane 3 has it)
	bsi.bA[3].Add(2)
	// Column 3: value is (1<<129) | (1<<3)
	bsi.bA[129].Add(3)
	bsi.bA[3].Add(3)
	// Column 4: value is 1<<3
	bsi.bA[3].Add(4)
