...
func (ra *roaringArray) readFrom(stream internal.ByteInput, cookieHeader ...byte) (int64, error) {
	...
	if !ra.checkKeysSorted() {
		return stream.GetReadBytes(), ErrKeySortOrder
	}
...
func keysAreSorted(keys []uint16) bool {
	if len(keys) < 2 {
		return true
	}
	previous := keys[0]
	for nextIdx := 1; nextIdx < len(keys); nextIdx++ {
		next := keys[nextIdx]
		if previous >= next {
			return false
		}
