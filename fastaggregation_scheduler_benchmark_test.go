...
// This file is proof-only benchmark material. It keeps the three scheduler
// contenders independent of HeapXor for both arms and also runs the production
// HeapXor path over the same matrix as the guard selector.
...
// single contender per benchmark operation. The HeapXor row is the durable
// aggregate guard selector; BenchmarkSchedulerComparison retains every cell
// for native per-regime inspection and CPU profiling.
...
func TestCompareSchedulerCorrectness(t *testing.T) {
...
func TestCompareSchedulerMixedContainers(t *testing.T) {
